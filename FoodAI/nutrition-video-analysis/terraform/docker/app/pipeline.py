...
class NutritionVideoPipeline:
    ...
    def _estimate_volume_from_raw_depth_with_gemini(
        ...
        """Estimate per-ingredient volumes using RGB + 3-D GLB (v2) or calibrated depth (v1).

        When glb_s3_key is provided (v2 pipeline), the GLB is uploaded to Gemini Files
        API and used instead of the calibrated depth image. This gives Gemini a full 3-D
        mesh from which to reason about ingredient volumes.
        """
        try:
            import google.generativeai as genai
            genai.configure(api_key=self.config.GEMINI_API_KEY)
        ...
        glb_file_ref = None
        if glb_s3_key:
            # v2: upload GLB from S3 to Gemini Files API
            import boto3, tempfile, os
            try:
                s3 = boto3.client("s3", region_name=self.config.TRELLIS_AWS_REGION)
                with tempfile.NamedTemporaryFile(suffix=".glb", delete=False) as tmp:
                    tmp_path = tmp.name
                s3.download_file(self.config.TRELLIS_OUTPUT_BUCKET, glb_s3_key, tmp_path)
                glb_file_ref = genai.upload_file(tmp_path, mime_type="model/gltf-binary")
                logger.info("[%s] Uploaded GLB to Gemini Files API: %s", job_id, glb_file_ref.name)
            except Exception as e:
                logger.warning("[%s] GLB upload to Gemini failed, falling back to depth image: %s", job_id, e)
                glb_file_ref = None
            finally:
                try:
                    os.unlink(tmp_path)
        ...
        if glb_file_ref:
            prompt = (
                ...
                "  Input 2: a 3-D GLB mesh of the dish generated by a photogrammetry model.\n\n"
                f"{scale_note}\n\n"
                + anchor_block +
                "Use the 3-D mesh geometry to estimate the volume of each ingredient by reasoning about "
                "the shape, height, and area visible in the mesh.\n\n"
