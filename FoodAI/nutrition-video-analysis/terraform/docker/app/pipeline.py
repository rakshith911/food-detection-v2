...
class NutritionVideoPipeline:
    ...
    def _verify_visible_rag_match_with_gemini(
        ...
        prompt = (
            ...
            "This food has already been cooked and served as part of a meal — it is on a plate right now.\n\n"
            "CRITICAL RULES — read before choosing:\n"
            "1. NEVER select entries marked 'dry', 'unprepared', 'uncooked', 'dry mix', 'packet', 'raw', "
            "'instant', 'powder', or 'dehydrated'. These are pre-cooking ingredient forms and give "
            "calorie values 2-3× too high for what is actually on the plate.\n"
            "2. If no exact cooked version of the food exists in the candidate list, select the CLOSEST "
            "COOKED EQUIVALENT by ingredients and preparation style. "
            "Example: 'yellow rice' has no exact cooked entry → pick 'Rice, white, long-grain, regular, "
            "enriched, cooked' or a similar cooked white/seasoned rice entry.\n"
            "3. Use the calorie values shown to sanity-check your pick: cooked rice/pasta/grains are "
            "typically 100-160 kcal/100g; cooked legumes 100-140 kcal/100g. "
            "If a candidate shows >250 kcal/100g for a starch or grain, it is almost certainly a dry form — reject it.\n"
            "4. Do NOT hallucinate a new entry. You must return one exact description string from the "
            "candidate list only.\n"
            "5. First judge the image on its own. Then compare against the candidate descriptions.\n\n"
            ...
            "- In your reason, explicitly state whether you are using an exact match or a cooked equivalent.\n"
        )
