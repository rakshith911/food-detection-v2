...
export class NutritionAnalysisAPI {
  ...
  async getResults(jobId: string, detailed: boolean = true, fetchDetailedJson: boolean = true): Promise<NutritionAnalysisResult | null> {
    try {
      ...
      const data = await response.json();
      console.log('[Nutrition API] Results received, status:', data.status, 'has_segmented_images:', !!data.segmented_images);
      ...
      if (fetchDetailedJson && data.download_url) {
        try {
          ...
          if (detailsResponse.ok) {
            ...
            // Extract meal name — check all known locations across both production and legacy pipelines
            const mealName =
              // production pipeline: top-level or production_debug
              detailedResults.meal_name ||
              detailedResults.production_debug?.meal_name ||
              // production pipeline via process_media wrapper
              detailedResults.full_results?.production_debug?.meal_name ||
              detailedResults.full_results?.meal_name ||
              // legacy pipeline: Gemini first-pass stored in gemini_outputs or florence_detections
              (detailedResults.full_results?.gemini_outputs as any[])?.[0]?.parsed_output?.meal_name ||
              (detailedResults.gemini_outputs as any[])?.[0]?.parsed_output?.meal_name;
            if (mealName) {
              data.meal_name = mealName;
              console.log('[Nutrition API] Extracted meal_name:', mealName);
            }

            // Extract segmented images — prefer top-level data.segmented_images (fresh presigned
            // URLs generated by Lambda on this request) over any stored in the result JSON
            // (those may have been generated by the ECS worker with STS credentials that have
            // already expired or had an invalid ExpiresIn > 604800s).
            if (!data.segmented_images && detailedResults.segmented_images?.overlay_urls?.length) {
              data.segmented_images = detailedResults.segmented_images;
              console.log('[Nutrition API] Segmented images extracted from stored JSON:', detailedResults.segmented_images.overlay_urls.length, 'overlays');
            }
          } else {
  ...
  async analyzeVideo(
    ...
    try {
      ...
      // 72 attempts × 5s = 360 seconds (6 minutes) before timeout
      onProgress?.('Processing video...');
      return await this.pollForResults(uploadData.job_id, onProgress, 72, 5000, true);
    } catch (error: any) {
  ...
  async analyzeImage(
    ...
    try {
      ...
      // Step 4: Poll for results (always request detailed to get segmented images)
      // 72 attempts × 5s = 360 seconds (6 minutes) before timeout — matches video timeout
